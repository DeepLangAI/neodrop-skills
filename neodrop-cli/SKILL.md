...
| "帮我收集 / 整理 / 追踪 …… 的信息" | 同上 |
| "我订阅了什么 / 我的频道更新了什么" | `channels list --mine` + `grains list --subscribed` |
| 用户贴 neodrop.ai/channel/<id> 或 /grain/<id> URL | 从 URL 抽 id 调 `channels get` / `grains get` |
...
2. 由 AI **自主决定下次回来的时机**（首次建议 8 分钟，未到终态时再延 3-5 分钟轮询一次），
   每次回来调 `neodrop tasks get <taskId>` 看 `status` + `channel._count.grains`：
   - `status == 'RUNNING'` 且 `grains == 0` → 还在跑，继续等
   - `status == 'RUNNING'` 且 `grains >= 1` → **频道已激活，首篇已出**——可以直接告诉用户结果
     了（task 终态延迟 1-3 分钟）
...
判断标准用 `grains > 0` 比 `status == COMPLETED` 更早——后端的 task 终态比首篇 grain 出现要
