...
| 浏览器打开授权页报「授权请求不合法 / 缺少有效的 session 参数」 | 打开的 URL 不完整——`login` 打印的授权 URL 含 `?session=...`、必然超一行被终端折行，复制（或点终端里的链接）时只取到前半段、漏了 `?session=` 尾部 | 重新 `login`；复制时把 `https://` 到 `session=` 后那串字符整段选上（连整行一起复制最稳），粘贴后确认浏览器地址栏里有 `?session=cas_...` 再回车。链接 10 分钟有效，旧的过期需重发 |
