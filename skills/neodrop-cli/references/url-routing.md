...
| `neodrop.ai/search?q=...` | 全站搜索 | `channels search "<q>"` + `grains search "<q>"` 综合呈现 |

## 反向：拿到 id 怎么给用户回链接

**不要凭记忆拼**——`grains get` / `channels get` / `me` 已经会在 stderr 打印一行 `🔗 <canonical-url>`，直接把那条引用给用户。

不要拼 `/grain/<id>`（前端没这个路由，老 `/grain` 已迁移到 `/feed`）或猜其它路径。如果命令没打印 canonical URL，按上表反推：

- channelId → `https://neodrop.ai/channel/<id>`
- grainId → `https://neodrop.ai/feed/<id>`
